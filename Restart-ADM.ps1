...
function invoke-ascurrentuser {
    ...
    if ($encodedcommand.length -gt $MaxLength -and $CacheToDisk -eq $false) {
        Write-Error -Message "The encoded script is longer than the command line parameter limit. Please execute the   
script with the -CacheToDisk option."
    ...
    if ($UseMicrosoftPowerShell -and -not (Test-Path -Path "$env:ProgramFiles\PowerShell\7\pwsh.exe")) {
        Write-Log -Message "Not able to find Microsoft PowerShell v7 (pwsh.exe). Ensure that it is installed on      
this system"
        return
    }
    ...
    if (-not $privs -or ($privs -band [RunAsUser.PrivilegeAttributes]::Disabled)) {
        Write-Log -Message "Not running with correct privilege. You must run this script as system or have the       
SeDelegateSessionUserImpersonatePrivilege token."
        ...
        try {
            ...
            Write-Log -Message "Could not execute as currently logged on user: $($_.Exception.Message)"
            return
        }
    }
}
